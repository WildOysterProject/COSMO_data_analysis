...
####Input####
COSMO_Input <-
  read_xlsx("Input/COSMO Data Entry MASTER 09.01.2026.xlsx", sheet = 1)

####Processing####
#####Initial Processing#####
COSMO_Processed <- COSMO_Input %>%
  mutate(
    across(Live_Oysters:Total_percent_cover, ~ as.numeric(.x)),
...
Cosmo_Count_Density_Pivoted <- COSMO_Processed_Filtered %>%
  ...
  pivot_longer(
    cols = Live_Oysters:Live_Mussels,
    names_to = "Sessile Organism",
    values_to = "Count"
  ) %>%
  mutate(
    Density = Count / `Oyster_count_quadrat_area_(m²)`,
...
COSMO_Normalized_Percentage <- COSMO_Processed_Filtered %>%
  ...
  mutate(
    across(Primary_percent_cover:UNID_spp., ~ .x * (Error_Score) * 4),
...
Percent_Cover_unpivoted <- COSMO_Normalized_Percentage %>%
  dplyr::select(SampleID:`Error_Score_(Qualitative)`,
                Error_Score,
                Bare:UNID_spp.)

Percent_Cover_unpivoted_trimmed <- Percent_Cover_unpivoted %>%
  filter(`Error_Score_(Qualitative)` != "High" &
           between(Error_Score, 0.8, 1.2)) #20% margin of error
...
Percent_Cover_aggregated <- Percent_Cover_unpivoted_trimmed %>%
  ...
  mutate(
    ...
    `Green Algae spp. canopy` = rowSums(dplyr::select(., all_of(
      `Green Algae spp. canopy`
    )), na.rm = T),
    `Red/Brown Algae spp.` = rowSums(dplyr::select(., all_of(
      `Red/Brown Algae spp.`
    )), na.rm = T),
    `Red/Brown Algae spp. canopy` = rowSums(dplyr::select(., all_of(
      `Red/Brown Algae spp. canopy`
    )), na.rm = T),
    `Bryozoan spp.` = rowSums(dplyr::select(., all_of(`Bryozoan spp.`)), na.rm = T),
    `Bryozoan spp. canopy` = rowSums(dplyr::select(., all_of(
      `Bryozoan spp. canopy`
    )), na.rm = T),
    `Tunicate spp.` = rowSums(dplyr::select(., all_of(`Tunicate spp.`)), na.rm = T),
    `Tunicate spp. canopy` = rowSums(dplyr::select(., all_of(
      `Tunicate spp. canopy`
    )), na.rm = T),
...
perquad_Native_Summary <- Primary_Percent_Cover_aggregated_pivoted %>%
  ...
  summarise(
    MeanPercentCover = mean(percent_cover, na.rm = T),
  ...
  pivot_longer(
    cols = c(
      MeanNativeProportion,
      `MeanNon-nativeProportion`,
      UnknownProportion
    ),
    names_to = c("Native"),
...
COSMO_OysterSizes <- COSMO_Processed %>%
  dplyr::select(
    !(Live_Oysters:Notes) &
      !c(
        `Error_Score_(Qualitative)`,
        `Oyster_count_quadrat_area_(m²)`
      )
  ) %>%
...
Primary_Percent_Cover_plot_data <- Primary_Percent_Cover_aggregated_pivoted %>%
  group_by(response_variable, Year) %>%
...
Primary_perquad_Percent_Cover_Native_Summary_barplot <- perquad_Native_Summary %>%
  ggplot(mapping = aes(x = Site, y = Proportion, fill = Native)) +
  geom_col(
    aes(group = Native),
  ...
  geom_errorbar(
    aes(ymin = Proportion - seProportion, ymax = Proportion + seProportion),
    width = 0.1,
    alpha = 0.5,
    position = position_dodge(preserve = "single", width = .9),
    alpha = 0.8
  ...
  scale_fill_manual(name = "Cover Type", values = nativecolors) +
...
ggsave(
  path = "Plots",
  filename = "Per-Quadrat Native Summary Barplot.jpg",
  width = 10,
  height = 10
)
...
Oyster_Size_Histogram <- COSMO_OysterSizes %>%
  ggplot(mapping = aes(x = Size, fill = Site)) +
  geom_histogram(
    color = 'white',
    binwidth = 10,
...
ggsave(
  path = "Plots",
  filename = "Oyster Size Histogram.jpg",
  width = 10,
  height = 10
)

Oyster_Size_Histogram_Seasonal <- Oyster_Size_Histogram +
  facet_grid(Site ~ Year + Season, scales = "free_y")
Oyster_Size_Histogram_Seasonal
ggsave(
  path = "Plots",
  filename = "Oyster Size Seasonal Histogram.jpg",
  width = 10,
  height = 10
)

#####Count Density Boxplot#####
Count_Density_Boxplot <- Cosmo_Count_Density_Pivoted %>%
  ggplot(mapping = aes(x = Site, y = Density, fill = `Sessile Organism`)) +
...
ggsave(
  path = "Plots",
  filename = "Sessile Organism Density vs. Site Boxplot.jpg",
  width = 10,
  height = 10
)

Count_Density_Boxplot_Seasonal <- Count_Density_Boxplot +
  facet_grid( ~ Year + Season, scales = "free")
Count_Density_Boxplot_Seasonal
ggsave(
  path = "Plots",
  filename = "Sessile Organism Density vs. Site vs. Season Boxplot.jpg",
  width = 10,
  height = 10
)
...
#   distinct(response_variable) #Run to see distinct response variables for scale_x_discrete

Site_Primary_Percent_Cover_Boxplot <- Primary_Percent_Cover_plot_data %>%
  ggplot(mapping = aes(x = response_variable, y = percent_cover)) +
...
ggsave(
  path = "Plots",
  filename = "Primary Cover vs Year vs Site boxplot.jpg",
  width = 10,
  height = 10
)
...
Diversity_Combined_Plot <- Percent_Cover_diversity %>%
  ggplot(mapping = aes(x = Year, y = mean, color = Site)) +
  ...
  geom_errorbar(aes(ymin = mean - se, ymax = mean + se),
                ...
                alpha = 0.5) +
  labs(y = "", x = "Year") +
  scale_fill_manual(values = sitecolors, aesthetics = c("color", "fill")) +
...
ggsave(
  path = "Plots",
  filename = "Combined Diversity Indices 2025-2026.jpg",
  width = 8,
  height = 6
)
...
####Lists####
#####Sites#####
Site_List <- c("SP", #Strawberry Point
               "HH", #Herons Head Park
               "PM", #Point Molate
               "PE", #Point Emery
...
#####Counts#####
Count_List <- c("Live Oysters", "Box Oysters", "Live Mussels")
...
`Green Algae spp.` <- c(
  ...
  "Ulva_intestinalis",
  "Green_Algae_sp.",
  "Green_filamentous_algae",
  "Cladophora",
  "Bryopsis"
)
`Green Algae spp. canopy` <- c(
  "Ulva_canopy",
  "Ulva_intestinalis_canopy",
  "Green_alg_sp._canopy",
  "Green_filamentous_algae_canopy",
  "Cladophora_canopy"
)
`Red/Brown Algae spp.` <- c(
  "Mastocarpus",
  "Red_Algae_sp.",
  "Caulacanthus",
  "Chondracanthus",
  "Polyneura",
  "Red_filamentous_algae",
  "Fucus_spp.",
  "Gracilaria_spp.",
  "Cryptopleura_ruprechtiana",
  "Cryptopleura_spp.",
  "Mazzaela_splendens",
  "Gymnogongrus_spp.",
  "Polysiphonia_spp."
)
`Red/Brown Algae spp. canopy` <- c(
  "Mastocarpus_canopy",
  "Red_alg_sp._canopy",
  "Caulacanthus_canopy",
  "Chondracanthus_canopy",
  "Polyneura_canopy",
  "Red_filamentous_algae_canopy",
  "Cryptopleura_ruprechtiana_canopy",
  "Cryptopleura_spp_canopy",
  "Mazzaela_splendens_canopy",
  "Fucus_canopy",
  "Gracilaria_canopy",
  "Gymnogongrus_spp_canopy",
  "Polysiphonia_canopy"
)
`Bryozoan spp.` <- c(
  "Bryozoan",
  ...
  "Watersipora_spp.",
  "Schizoporella_spp.",
  "Encrusting_bryozoan_spp.",
  "Upright_bryozoan_spp."
)
`Bryozoan spp. canopy` <- c(
  "Bryozoan_canopy",
  "Bugula_neritina_canopy",
  "Encrusting_bryozoan_canopy",
  "Upright_bryozoan_canopy"
)
`Tunicate spp.` <- c("Tunicate",
                     "Colonial_tunicate",
                     "Solitary_tunicate",
                     "Botrylloides_spp.")
...
responsevarcolors <- c(
  `Bare` = "grey",
  `Barnacles` = "#A5EDFF",
  `Barnacle canopy` = "#A5EDFF",
  `Bryozoan spp.` = "#FFFF99",
  `Bryozoan spp. canopy` = "#FFFF99",
  `Diatom sp.` = "#FFBF7F",
  `diatom_canopy` = "#FFBF7F",
  `Green Algae spp.` =  "#32FF00",
  `Green Algae spp. canopy` =  "#32FF00",
  `Hydroid` = "#FF7F00",
  `Hydroid_canopy` = "#FF7F00",
  `Mussels` = "#654CFF",
  `Oysters` = "#19B2FF",
  `Oyster_canopy` = "#19B2FF",
  `Red/Brown Algae spp.` = "#E51932",
  `Red/Brown Algae spp. canopy` = "#E51932",
  `Sponge spp.` = "gold",
  `Sponge spp. canopy` = "gold",
  `Tubeworm` = "#CCBFFF",
  `Tunicate spp.` = "#FF99BF",
  `Tunicate spp. canopy` = "#FF99BF",
  `Zostera_marina` = "darkgreen" #change?
)
